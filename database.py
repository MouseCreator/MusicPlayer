from model.music import UserCache, MusicSettings, Playlist, MusicState, RepeatOption
...
class DataBase:
    ...
    def update_playback(self, playback: MusicState | None = None):
        if playback is not None:
            self._settings._state = playback
        else:
            prev = self._settings._state
            if prev == MusicState.PLAYING:
                self._settings._state = MusicState.PAUSED
            if prev == MusicState.PAUSED:
                self._settings._state = MusicState.PLAYING
        self._event_registry.register("playback", PlaybackChangedEvent(self._settings._state))
    ...
    def update_repeat(self, repeat_option: RepeatOption | None = None):
        if repeat_option is not None:
            self._settings._state = repeat_option
        else:
